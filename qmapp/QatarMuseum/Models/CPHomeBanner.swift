...
//  QatarMuseums
//
//  Created by Wakralab Software Labs on 27/11/18.
//  Copyright © 2018 Qatar Museums. All rights reserved.
//

import Foundation
struct CPHomeBanner: CPResponseObjectSerializable, CPResponseCollectionSerializable {
...
struct HomeBannerList: CPResponseObjectSerializable {
    var homeBannerList: [CPHomeBanner]? = []
    
    public init?(response: HTTPURLResponse, representation: AnyObject) {
        if let data = representation as? [[String: Any]] {
            self.homeBannerList = CPHomeBanner.collection(response: response, representation: data as AnyObject)
        }
    }
}
