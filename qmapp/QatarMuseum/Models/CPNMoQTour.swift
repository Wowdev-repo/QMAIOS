...
import Foundation
struct CPNMoQTour: CPResponseObjectSerializable, CPResponseCollectionSerializable {
...
struct NMoQTourList: CPResponseObjectSerializable {
    var nmoqTourList: [CPNMoQTour]? = []
    
    public init?(response: HTTPURLResponse, representation: AnyObject) {
        if let data = representation as? [[String: Any]] {
            self.nmoqTourList = CPNMoQTour.collection(response: response, representation: data as AnyObject)
        }
    }
}
