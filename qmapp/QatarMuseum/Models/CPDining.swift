...
//  QatarMuseums
//
//  Created by Wakralab Software Labs on 02/08/18.
//  Copyright © 2018 Qatar Museums. All rights reserved.
//

import Foundation
struct CPDining: CPResponseObjectSerializable, CPResponseCollectionSerializable {
    var name: String? = nil
    var id: String? = nil
...
struct Dinings: CPResponseObjectSerializable {
    var dinings: [CPDining]? = []
    
    public init?(response: HTTPURLResponse, representation: AnyObject) {
        if let data = representation as? [[String: Any]] {
            self.dinings = CPDining.collection(response: response, representation: data as AnyObject)
        }
    }
}
