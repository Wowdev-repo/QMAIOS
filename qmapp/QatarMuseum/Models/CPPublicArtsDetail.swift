//
//  CPPublicArtsDetail.swift
//  QatarMuseums
//
//  Created by Wakralab Software Labs on 05/08/18.
//  Copyright © 2018 Qatar Museums. All rights reserved.
//

import Foundation
struct CPPublicArtsDetail: CPResponseObjectSerializable, CPResponseCollectionSerializable {
    var id: String? = nil
    var name: String? = nil
...
struct PublicArtsDetails: CPResponseObjectSerializable {
    var publicArtsDetail: [CPPublicArtsDetail]? = []
    
    public init?(response: HTTPURLResponse, representation: AnyObject) {
        if let data = representation as? [[String: Any]] {
            self.publicArtsDetail = CPPublicArtsDetail.collection(response: response, representation: data as AnyObject)
        }
    }
}
