...
struct CPCollection: CPResponseObjectSerializable, CPResponseCollectionSerializable {
    var name: String? = nil
...
struct Collections: CPResponseObjectSerializable {
    var collections: [CPCollection]? = []
    
    public init?(response: HTTPURLResponse, representation: AnyObject) {
        if let data = representation as? [[String: Any]] {
            self.collections = CPCollection.collection(response: response, representation: data as AnyObject)
        }
    }
}
