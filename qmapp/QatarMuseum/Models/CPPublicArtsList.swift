...
import Foundation
struct CPPublicArtsList: CPResponseObjectSerializable, CPResponseCollectionSerializable {
    var id: String? = nil
    var name: String? = nil
...
struct PublicArtsLists: CPResponseObjectSerializable {
    var publicArtsList: [CPPublicArtsList]? = []
    
    public init?(response: HTTPURLResponse, representation: AnyObject) {
        if let data = representation as? [[String: Any]] {
            self.publicArtsList = CPPublicArtsList.collection(response: response, representation: data as AnyObject)
        }
    }
}
