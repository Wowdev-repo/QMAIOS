//
//  CPMapDetailView.swift
//  QatarMuseums
//
//  Created by Wakralab Software Labs on 10/09/18.
...
class CPMapDetailView: UIViewController,ObjectImageViewProtocol {
    ...
    var mapdetailDelegate : CPMapDetailProtocol?
    let closeButton = UIButton()
    var objectImagePopupView : CPObjectImageView = CPObjectImageView()
    var gesture = UIPanGestureRecognizer()
    var popUpArray: [CPTourGuideFloorMap]! = []
...
//        tableView.dataSource = self
        ...
        tableView.register(UITableViewCell.self, forCellReuseIdentifier: "imageCell")
         ...
         gesture = UIPanGestureRecognizer.init(target: self, action: #selector(CPMapDetailView.panGesture))
...
extension CPMapDetailView: UITableViewDelegate, UITableViewDataSource {
...
//                selectedCell?.timer?.invalidate()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if (indexPath.row == 0) {
            let cell = tableView.dequeueReusableCell(withIdentifier: "objectPopupId", for: indexPath) as! CPObjectPopupTableViewCell
           ...
           // return tableView.dequeueReusableCell(withIdentifier: "objectPopupId")!
            ...
            return cell
             ...
             let cell = tableView.dequeueReusableCell(withIdentifier: "objectDetailID", for: indexPath) as! CPObjectDetailTableViewCell
            cell.selectionStyle = .none
            if(selectedIndex != nil) {
                if (indexPath.row == 2){
    ...
    func setPlayButtonAction(cellObj: CPObjectDetailTableViewCell, index: Int) {
...
extension CPMapDetailView: UIGestureRecognizerDelegate {
    ...
    @objc func loadObjectImagePopup(imgName: String?) {
        ...
        objectImagePopupView = CPObjectImageView(frame: frameRect)
        objectImagePopupView.objectImageViewDelegate = self
