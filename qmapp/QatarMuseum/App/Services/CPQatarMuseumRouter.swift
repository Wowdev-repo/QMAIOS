...
enum CPQatarMuseumRouter: URLRequestConvertible {
    ...
    public var request: URLRequest {
        let URL = NSURL(string: CPConfig.baseURL + lang() + CPConfig.mobileApiURL)!
        ...
        case .ExhibitionList(let apiLang):
            let apiURL = NSURL(string: CPConfig.baseURL + apiLang + CPConfig.mobileApiURL)!
        ...
        case .GetToken(let parameters):
            let tokenURL = NSURL(string: CPConfig.baseURL + lang() + CPConfig.mobileApiURL)!
        ...
        case .Login(let parameters):
            let loginURL = NSURL(string: CPConfig.baseURL + lang() + CPConfig.mobileApiURL)!
        ...
        case .Logout():
            let logoutURL = NSURL(string: CPConfig.baseURL + lang() + CPConfig.mobileApiURL)!
        ...
        case .NewPasswordRequest(let parameters):
            let newPasswordURL = NSURL(string: CPConfig.baseURL + lang() + CPConfig.mobileApiURL)!
        ...
        case .SendDeviceToken(let accessToken, let parameters):
            let deviceTokenURL = NSURL(string: CPConfig.baseURL + lang() + CPConfig.mobileApiURL)!
            ...
            return try! Alamofire.JSONEncoding.default.encode(tokenMutableURLReq, with: parameters)
        case .NumberSearchList( _):
            let searchURL = NSURL(string: CPConfig.baseURL + lang() + CPConfig.mobileApiURL)!
        ...
        case .NMoQEntityRegistration(let parameters):
            let regURL = NSURL(string: CPConfig.baseURL + lang() + CPConfig.mobileApiURL)!
        ...
        case .SetUserRegistrationComplete( _,let parameters):
            let regURL = NSURL(string: CPConfig.baseURL + CPConfig.engLang + CPConfig.mobileApiURL)!
            var regMutableURLReq = URLRequest(url: regURL.appendingPathComponent(path)!)
            regMutableURLReq.httpMethod = method.rawValue
            if let accessToken = UserDefaults.standard.value(forKey: "accessToken") as? String {
                regMutableURLReq.setValue(accessToken, forHTTPHeaderField: "x-csrf-token")
