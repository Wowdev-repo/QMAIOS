...
//  QatarMuseums
//
//  Created by Wakralab on 23/06/18.
...
class CPMuseumsViewController: UIViewController {
        ...
        if(fromHomeBanner)! {
            ...
            if ((selectedItem == "About") || (selectedItem == "عن")) {
                ...
                if((museumId == "66") || (museumId == "638")) {
                    ...
                    let parkView = self.storyboard?.instantiateViewController(withIdentifier: "heritageDetailViewId") as! CPCommonDetailViewController
                    parkView.pageNameString = CPPageName.SideMenuPark
                    let transition = CATransition()
                    transition.duration = 0.3
                    transition.type = kCATransitionFade
                    transition.timingFunction = CAMediaTimingFunction(name:kCAMediaTimingFunctionEaseInEaseOut)
...
extension CPMuseumsViewController: comingSoonPopUpProtocol {
...
extension CPMuseumsViewController: TopBarProtocol {
...
//MARK:- KASlideShow delegates
extension CPMuseumsViewController: KASlideShowDelegate {
    ...
    func setSlideShow(imgArray: NSArray) {
        ...
        pageControl.currentPage = Int(museumsSlideView.currentIndex)
        pageControl.addTarget(self, action: #selector(CPMuseumsViewController.pageChanged), for: .valueChanged)
    }
}
